// ===== МОДУЛЬ КОНФИГУРАЦИИ КОЛОНОК СООБЩЕНИЙ =====

window.MessagesColumns = (function() {
    ...
    function loadColumnsConfig() {
        try {
            ...
            console.error('Ошибка загрузки конфигурации колонок сообщений:', e);
    ...
    function getDefaultColumnsConfig() {
        return {
            ...
            'time': {'name': 'Time', 'visible': true, 'order': 1},
            'type': {'name': 'Type', 'visible': true, 'order': 2},
            'message': {'name': 'Message', 'visible': true, 'order': 3}
        };
    }
    ...
    async function saveColumnsConfig() {
        try {
            const result = await API.post('/messages/save_columns_config', columnsConfig);
            if (result.status === 'success') {
                console.log('✅ Конфигурация колонок сообщений сохранена');
            }
        } catch (e) {
            console.error('❌ Ошибка сохранения конфигурации сообщений:', e);
        }
    }
    
    async function resetColumns() {
        try {
            const result = await API.post('/messages/reset_columns');
            if (result.status === 'success') {
                columnsConfig = result.config;
                updateColumnsPanel();
                applyColumnVisibility();
                await window.MessagesTable.fetchMessages();
                Notifications.success('Конфигурация колонок сообщений сброшена');
            }
        } catch (e) {
            console.error('Ошибка сброса конфигурации сообщений:', e);
            Notifications.error('Ошибка сброса конфигурации сообщений');
        }
    }
